...
const DataManagement = () => {
  ...
  const completedFiles = datasets.filter(
    (d) => d.status === "completed"
  ).length;
  const totalRecords = datasets
    .filter((d) => d.status === "completed")
    .reduce((sum, d) => sum + d.value_count, 0);
  const totalEmbeddings = datasets
    .filter((d) => d.status === "completed")
    .reduce((sum, d) => sum + d.embedding_count, 0);

  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-black text-primary">Data Management</h1>
                    ...
                    <TableRow key={dataset.id}>
                      ...
                      <TableCell>
                        {dataset.status === "completed"
                          ? dataset.value_count.toLocaleString()
                          : "-"}
                      </TableCell>
                      <TableCell>
                        {dataset.status === "completed"
                          ? dataset.embedding_count.toLocaleString()
                          : "-"}
                      </TableCell>
                      <TableCell>
                        <div className="flex items-center gap-2">
